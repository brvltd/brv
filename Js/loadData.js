function getData(){
	...
	if (track_no=="") {
		...
		let ref=firebase.database().ref("Cargo/"+track_no);
		ref.once('value').then(function (snapshot) {
			var cargoName=snapshot.val().CargoName;
			var custName=snapshot.val().CustomerName;
			var trackN=snapshot.val().TrackNo;
			console.log(cargoName,custName,trackN);
			document.getElementById("id").innerHTML=trackN;
			document.getElementById("name").innerHTML=custName;
			document.getElementById("cargo").innerHTML=cargoName;
		});
